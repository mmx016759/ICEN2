...
public class 安慰之心 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (!技能id.安慰之心.GetSpell().IsReadyWithCanCast()) return -2;
        if (Helper.自身存在其中Buff(BuffBlackList.无法发动技能)) return -1;
        if (白魔Qt.GetQt("高难模式")&&白魔Qt.GetQt("安慰之心")) return 1;
        if (!白魔Qt.GetQt("安慰之心")) return -1;
        if (!白魔Qt.GetQt("单奶")) return -1;
        if (Core.Resolve<JobApi_WhiteMage>().Lily<= 0) return -2;
        if (Helper.自身当前等级 > 73 && Helper.白魔量谱_蓝花数量() > 2&&默认值.实例.蓝花防溢出) return 1;
        //获取符合血量阈值的人
        var list = PartyHelper.CastableAlliesWithin30
            .Concat([Core.Me])
            .Where(ally => ally.CurrentHpPercent() <= 默认值.实例.安慰之心血量 / 100f)
            .ToList();
        if (list.Count > 3) return -3; //超过两个人就不单奶
        var target = list
    ...
    {
        slot.Add(new Spell(技能id.安慰之心, Helper.获取血量最低成员()));
    }
}